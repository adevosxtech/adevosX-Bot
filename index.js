...
// --- Environment Setup ---
// MUST be first: silence known-noisy baileys/libsignal output before
// any other module loads and starts logging.
require('./lib/silenceNoise');
...
/**
 * Custom logging function to enforce the [ ADEVOS-X ] prefix and styling.
...
function log(message, color = 'white', isError = false) {
    const prefix = chalk.magenta.bold('[ ADEVOS-X BOT ]');
...
// Welcome message guard — set to true after the welcome lands once in this
// process. Lives only in memory, so it naturally resets on every restart
// (panel restart, .update, 408 retry, etc.) → exactly one welcome per restart.
...
// Debounced async write — never blocks the message handler
let _saveTimer = null;
function saveStoredMessages(data) {
    ...
    }, 3000); // batch writes, flush 3s after last message
}
...
// --- ADEVOS-X ORIGINAL CODE START ---
global.botname = "𝐀𝐝𝐞𝐯𝐨𝐬-𝐗 𝐁𝐨𝐭"
global.themeemoji = "•"
...
// Valid session ID prefixes — all are accepted
const VALID_PREFIXES = ['ADEVOS-X:~', 'DAVE-X:~', 'ADEVOS-X BOT:~', 'CYPHERX:~', 'MD:~'];
...
/**
 * NEW LOGIC: Checks if SESSION_ID has a valid prefix. If not, cleans .env and restarts.
 */
async function checkAndHandleSessionFormat() {
    ...
    if (sessionId && sessionId.trim() !== '') {
        ...
        if (!hasValidPrefix(sessionId.trim())) {
            ...
            log(chalk.white.bgRed('[SESSION ID] MUST start with "ADEVOS-X:~", "DAVE-X:~", etc.'), 'white');
...
async function getLoginMethod() {
    ...
    if (choice === '1') {
        ...
        if (phone.length < 7) {
            log('❌ Phone number too short. Please enter a valid international number (e.g., +000000000000).', 'red');
        ...
        let sessionId = await question(chalk.bgBlack(chalk.greenBright(`Paste your Session ID here (e.g. ADEVOS-X:~...): `)));
        sessionId = sessionId.trim();
        // Accept any known prefix
        if (!hasValidPrefix(sessionId)) { 
            log("Invalid Session ID! Must start with ADEVOS-X:~, DAVE-X:~, CYPHER-X:~ etc.", 'red'); 
            process.exit(1); 
        }
...
async function downloadSessionData() {
    try {
        ...
        if (!fs.existsSync(credsPath) && global.SESSION_ID) {
            // Extract base64 after any known prefix
...
async function sendWelcomeMessage(XeonBotInc) {
    // Hard guard — already delivered in this process, don't re-send on
    // reconnects within the same process lifetime.
    if (global.welcomeSent) {
        log('Welcome already delivered this session — skipping.', 'cyan');
        return;
    }
...
╰─────━━━━───────`
        ...
        // Mark welcome delivered for the rest of this process lifetime so
        // baileys reconnects don't re-spam the number. A fresh restart
        // resets this flag (in-memory) → next start delivers exactly one.
        ...
        const groupInvites = ["CSVGZbyTt6rJZbfFiWXPYK"];
...
async function startXeonBotInc() {
    ...
    const XeonBotInc = makeWASocket({
        ...
        keepAliveIntervalMs: 15000,   // ping WhatsApp every 15s to keep connection alive
        connectTimeoutMs: 20000,      // fail fast on stale connection
        defaultQueryTimeoutMs: 15000, // don't hang waiting for WA response
    ...
    XeonBotInc.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            log('ADEVOS-X CONNECTED', 'yellow');      
            log(`GITHUB: ADEVOS-X`, 'yellow');
